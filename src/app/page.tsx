...
function AnimatedBg() {
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
      ...
      <div
        className="absolute top-1/4 -right-32 w-[450px] h-[450px] bg-gradient-to-br from-pink-400/30 to-rose-400/20 rounded-full blur-3xl animate-blob"
        style={{ animationDelay: "2s" }}
      />
      <div
        className="absolute -bottom-32 left-1/4 w-[500px] h-[500px] bg-gradient-to-br from-cyan-400/25 to-sky-400/20 rounded-full blur-3xl animate-blob"
        style={{ animationDelay: "4s" }}
      />
      <div
        className="absolute top-1/2 right-1/3 w-[400px] h-[400px] bg-gradient-to-br from-amber-300/20 to-orange-300/15 rounded-full blur-3xl animate-blob"
        style={{ animationDelay: "6s" }}
      />
      <div
        className="absolute top-3/4 left-1/2 w-[350px] h-[350px] bg-gradient-to-br from-emerald-300/15 to-teal-300/10 rounded-full blur-3xl animate-blob"
      ...
      <div className="absolute top-[10%] right-[10%] text-5xl animate-float-reverse opacity-40">
        🌈
      </div>
      <div className="absolute top-[35%] left-[3%] text-3xl animate-float opacity-35">
        🎈
      </div>
      <div className="absolute top-[60%] right-[5%] text-4xl animate-float-reverse opacity-40">
        🦋
      </div>
      <div className="absolute bottom-[10%] left-[12%] text-3xl animate-float opacity-30">
        🌸
      </div>
      <div className="absolute top-[20%] right-[28%] text-2xl animate-float-reverse opacity-25">
        💫
      </div>
      <div className="absolute bottom-[20%] right-[18%] text-3xl animate-float opacity-35">
        🎵
      </div>
      <div className="absolute top-[50%] left-[22%] text-2xl animate-float-reverse opacity-25">
        ✨
      </div>
      <div className="absolute top-[75%] left-[40%] text-3xl animate-float opacity-20">
        🎀
      </div>
      <div className="absolute top-[15%] left-[45%] text-2xl animate-float-reverse opacity-20">
        🍭
      </div>
      ...
      <div
        className="absolute top-[45%] right-[15%] w-1.5 h-1.5 bg-pink-300 rounded-full animate-sparkle opacity-50"
        style={{ animationDelay: "1s" }}
      />
      <div
        className="absolute bottom-[30%] left-[8%] w-2 h-2 bg-purple-300 rounded-full animate-sparkle opacity-40"
        style={{ animationDelay: "2s" }}
      />
      <div
        className="absolute top-[70%] right-[35%] w-1.5 h-1.5 bg-cyan-300 rounded-full animate-sparkle opacity-50"
...
function ChildCard({ child, rank }: { child: ChildPublic; rank: number }) {
  ...
  return (
    <div className="bg-white/90 backdrop-blur rounded-[2rem] overflow-hidden shadow-xl hover:shadow-2xl transition-shadow duration-300 group border border-white/60">
      {/* Gradient header */}
      <div
        className={`bg-gradient-to-br ${headerGradients[gi]} p-7 text-white relative overflow-hidden`}
      >
        {/* Background pattern */}
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-2 left-4 text-6xl">{child.emoji}</div>
          <div className="absolute bottom-2 right-4 text-8xl -rotate-12">
            {child.emoji}
          </div>
          <div className="absolute top-1/2 left-1/2 text-5xl rotate-12">
            {child.emoji}
          </div>
        </div>
        ...
        {/* Crown for #1 */}
        {rank === 1 && (
          <div className="absolute top-3 right-4 text-3xl animate-pulse">
            👑
          </div>
        )}
        ...
        <div className="text-center pt-3 relative z-10">
          ...
          <div
            className={`inline-flex items-center gap-1.5 px-4 py-1.5 mt-3 mb-6 rounded-full bg-gradient-to-r ${level.gradient} text-sm font-bold shadow-lg`}
          >
        ...
        <div className="flex justify-center gap-2 mb-5">
          {Array.from({ length: 5 }).map((_, i) => {
            ...
            return (
              ...
              >
                <span
                  className={`text-lg ${filled ? "" : "grayscale opacity-30"}`}
                >
                  ⭐
                </span>
              </div>
...
function Podium({ sorted }: { sorted: ChildPublic[] }) {
  ...
  const display =
    top3.length === 3 ? [top3[1], top3[0], top3[2]] : [top3[1], top3[0]];

  const configs = [
    {
      pillarH: 160,
      pillarW: "w-32 md:w-40",
          ...
          return (
            <div key={child.id} className="text-center group">
              ...
              <div className="relative inline-block mb-3">
                ...
                {isFirst && (
                  <div
                    className="absolute -top-5 left-1/2 -translate-x-1/2 text-4xl drop-shadow-lg"
                ...
                {config.sparkle && (
                  <>
                    ...
                    <div
                      className="absolute top-1 -left-2 w-2 h-2 bg-amber-300 rounded-full animate-sparkle"
                      style={{ animationDelay: "0.5s" }}
                    />
                    <div
                      className="absolute -bottom-1 right-0 w-2.5 h-2.5 bg-yellow-200 rounded-full animate-sparkle"
                      style={{ animationDelay: "1s" }}
                    />
                  </>
                )}
              ...
              {/* Name + score */}
              <p
                className={`font-black text-gray-800 ${isFirst ? "text-xl" : "text-base"}`}
              >
                {child.name}
              </p>
              <span
                className={`inline-flex items-center gap-1 text-xs font-bold px-3 py-1 rounded-full bg-gradient-to-r ${level.gradient} text-white mt-1 mb-3 shadow-md`}
              >
              ...
              >
                ...
                <div className="relative z-10 flex flex-col items-center">
                  <span
                    className={`${isFirst ? "text-5xl" : "text-4xl"} drop-shadow-xl mb-1`}
                  >
                    {config.medal}
                  </span>
                  <span
                    className={`text-white font-black ${isFirst ? "text-2xl" : "text-lg"} drop-shadow`}
                  >
                    #{idx + 1}
                  </span>
                </div>
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-purple-200 text-base">
            Với {leader.totalPoints} điểm sau {totalActivities} hoạt động. Ai sẽ
            vượt lên?
          </p>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-100 via-pink-50 via-50% to-amber-50 relative">
      ...
      <header className="relative z-10 text-center pt-14 pb-10 px-4">
        <div className="inline-block relative mb-6">
          ...
          <div
            className="absolute -bottom-1 -left-2 text-2xl animate-bounce"
        ...
        {sorted.length > 0 && (
          <div className="flex justify-center gap-3 mt-8 flex-wrap">
            ...
            <span className="px-5 py-2.5 bg-white/80 backdrop-blur-sm rounded-full shadow-lg shadow-emerald-500/5 text-sm font-bold text-emerald-600 border border-emerald-100/50">
              ⭐ {sorted.reduce((s, c) => s + Math.max(c.totalPoints, 0), 0)}{" "}
              điểm
            </span>
            <span className="px-5 py-2.5 bg-white/80 backdrop-blur-sm rounded-full shadow-lg shadow-pink-500/5 text-sm font-bold text-pink-600 border border-pink-100/50">
              📝 {sorted.reduce((s, c) => s + c.recentActivities.length, 0)}{" "}
              hoạt động
            </span>
          </div>
        )}
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-4 pb-16">
        {loading ? (
          <div className="text-center py-28">
            ...
            <p className="text-gray-400 mt-6 font-bold text-xl">
              Đang tải bảng điểm...
            </p>
          </div>
        ) : sorted.length === 0 ? (
