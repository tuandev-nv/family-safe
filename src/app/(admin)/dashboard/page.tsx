...
export default function DashboardPage() {
  ...
  const fetchDashboard = useCallback(() => {
    setLoading(true);
    fetch(
      `/api/dashboard?year=${selectedMonth.year}&month=${selectedMonth.month}`,
    )
      .then((r) => {
        ...
        <div className="text-center py-16">
          ...
          <div className="flex gap-4 justify-center">
            <Button
              ...
              className="h-11 px-6 rounded-xl text-base font-bold bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 shadow-lg shadow-purple-500/25"
            >
              👶 Thêm trẻ em
            </Button>
            <Button
  ...
  const totalPenalties = data.children.reduce(
    (s, c) => s + Math.abs(c.penaltyPoints),
    0,
  );
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-white rounded-2xl p-5 border border-orange-100/60 shadow-sm">
          ...
          <p className="text-3xl font-extrabold text-rose-500 mt-3">
            -{totalPenalties}
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        ...
        >
          ...
          <p className="text-emerald-100 text-sm">
            Thưởng hoặc phạt cho các bé ngay!
          </p>
          <span className="inline-block mt-3 px-4 py-1.5 bg-white/20 backdrop-blur-sm rounded-full text-sm font-semibold">
            + Ghi nhận ngay
          </span>
        </button>
        <button
        ...
        >
          ...
          <p className="text-orange-100 text-sm">
            Tạo thêm tiêu chí thưởng/phạt mới
          </p>
          <span className="inline-block mt-3 px-4 py-1.5 bg-white/20 backdrop-blur-sm rounded-full text-sm font-semibold">
            Tùy chỉnh
          </span>
      ...
      <div className="mb-8">
        <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
          <span className="text-2xl">📅</span> Bảng điểm các bé -{" "}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {data.children.map((child, idx) => {
            ...
            const stars = Math.min(
              Math.floor(Math.max(child.monthlyPoints, 0) / 20),
              5,
            );
            return (
              <Link
              ...
              >
                <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-shadow duration-300">
                  <div
                    className={`bg-gradient-to-r ${gradients[gi]} p-5 text-white relative overflow-hidden`}
                  >
                    <div className="absolute -right-3 -top-3 text-7xl opacity-15">
                      {child.emoji}
                    </div>
                      ...
                      <div>
                        ...
                        <div className="flex gap-0.5 mt-1">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <span
                              key={i}
                              className={`text-xs ${i < stars ? "opacity-100" : "opacity-30"}`}
                            >
                              ⭐
                            </span>
                          ))}
                  ...
                  <div className="p-5">
                    <div className="text-center mb-3">
                      <span
                        className={`text-4xl font-extrabold ${child.monthlyPoints >= 0 ? "text-gray-800" : "text-rose-500"}`}
                      >
                        {child.monthlyPoints > 0 ? "+" : ""}
                        {child.monthlyPoints}
                      </span>
                      <span className="text-gray-400 ml-1.5 text-base">
                        điểm
                      </span>
                    </div>
